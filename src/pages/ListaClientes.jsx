...
import apiRequest from "../services/api"; // Ajusta la ruta según tu estructura de proyecto
...
const ListaClientes = () => {
  ...
  useEffect(() => {
    const fetchClientes = async () => {
      ...
      try {
        ...
        // Utilizando el componente apiRequest
        const data = await apiRequest("cliente");
        console.log("Respuesta completa:", data);
        
        // Procesar datos según la estructura de tu API Laravel
        // Laravel típicamente devuelve datos en formato: { data: [...] }
  ...
  const handleDelete = async (id, nombre) => {
    if (window.confirm(`¿Estás seguro de que deseas eliminar al cliente ${nombre}?`)) {
      try {
        // Usando el componente apiRequest para eliminar
        ...
        alert("No se pudo eliminar el cliente. " + (err.message || "Podría estar asociado a otros registros."));
      }
    }
  };
